...
- **Controls / details (default)**: open the card’s controls, such as brightness and color for a light. Cards without a control menu show read-only details.
...
Save the card after editing. These settings work on the home screen and inside subpages, and are included in backups. A long press opens its menu once without running the tap action, including when you release the card. The next ordinary tap works normally. Closing the information returns to the current page.
