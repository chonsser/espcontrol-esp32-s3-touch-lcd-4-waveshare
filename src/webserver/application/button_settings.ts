...
export function createButtonSettingsFeature(
    ...
    function renderButtonSettings(this: any, forceOpen?: any) {
        ...
        if (cardSupportsLongPress(b)) {
            const holdAction = selectField("Long Press", idPrefix + "long-press", [
                ["", "Same as tap"], ["more_info", "Show more info"], ["none", "No action"],
            ...
            if (longPressAction(b.options) === "more_info") {
                const infoEntity = entityField("Info Entity (optional)", idPrefix + "long-press-entity",
                    configOptionValue(b.options, "long_press_entity"),
                    longPressDefaultEntity(b) || "e.g. sensor.living_room_temperature", []);
                panel.appendChild(infoEntity.field);
                requireField(infoEntity.input, "Use an entity ID such as sensor.living_room_temperature.",
                    undefined, (value: string) => !value.trim() || /^[a-z_]+\.[a-z0-9_]+$/.test(value.trim()));
                const infoText = textField("Additional Information (optional)", idPrefix + "long-press-text",
                    configOptionValue(b.options, "long_press_text"), "Text to show on the panel");
                ...
                hint.textContent = "Hold the card to show its name and current state. Leave Info Entity empty to use the card's entity. A long press does not run the tap action.";
