...
window.start = async (options = {}) => {
  ...
  const render = () => {
    ...
    const target = runtime.els.previewMain;
  ...
  const shell = createControlsShellFeature(runtime, { document, state, buildScreenToolbar: navigation.buildToolbar, buildScreenSettings: navigation.buildCard, closeSettings() {} });
...
(async () => {
  ...
  try {
    ...
    await raw.locator("#sp-screen-picker").selectOption("1");
    await raw.getByLabel("Screen name", { exact: true }).fill("New name");
    await raw.getByRole("button", { name: "Rename", exact: true }).click();
    await raw.waitForFunction(() => window.server.subpages[1] === "@screen:New name\n");
    await raw.locator("#sp-screen-picker").selectOption("0");
    await raw.evaluate(async () => { await window.restoreNative({ 1: "@screen:Restored%20name\n" }); });
    await raw.locator("#sp-screen-picker").selectOption("1");
    await raw.getByLabel("Screen name", { exact: true }).fill("After restore");
    await raw.getByRole("button", { name: "Rename", exact: true }).click();
    ...
    await raw.getByLabel("Screen name", { exact: true }).fill("After generic save");
    await raw.getByRole("button", { name: "Rename", exact: true }).click();
    await raw.waitForFunction(() => window.server.subpages[1] === "@screen:After generic save\n");
    await raw.getByRole("button", { name: "Delete screen", exact: true }).click();
    ...
    assert.equal(await missing.locator("#sp-screen-picker option[value='4']").count(), 0);
    ...
    await locale.locator("#sp-screen-picker").selectOption("1");
    await locale.getByLabel("Screen name", { exact: true }).fill("Name draft");
    ...
    assert.equal(await locale.getByLabel("Screen name", { exact: true }).inputValue(), "Name draft");
    const reloaded = locale.waitForEvent("framenavigated");
    await locale.getByRole("button", { name: "Rename", exact: true }).click();
    await reloaded;
    ...
    await busyLocale.locator("#sp-screen-picker").selectOption("1");
    await busyLocale.evaluate(() => { window.holdPut = true; });
    await busyLocale.getByRole("button", { name: "Rename", exact: true }).click();
    ...
    const picker = page.locator("#sp-screen-picker");
    ...
    assert.equal(await add.evaluate(el => el.previousElementSibling.id), "sp-screen-picker", "plus is adjacent");
    ...
    assert.equal(await page.getByLabel("State value 1", { exact: true }).inputValue(), "Media");
    assert.equal(await page.getByLabel("State value 2", { exact: true }).inputValue(), "Radio");
    await page.getByLabel("State value 2", { exact: true }).fill("Radio draft");
    await picker.selectOption("0"); await picker.selectOption("2");
    assert.equal(await page.getByLabel("State value 2", { exact: true }).inputValue(), "Radio draft");
    await add.click();
    await page.waitForFunction(() => window.state.editingSubpage === 1);
    assert.equal(await page.locator(".fixture-cell").count(), 4, "independent grid uses all home-size cells");
    assert.equal(await page.locator(".fixture-cell").filter({ hasText: "Back" }).count(), 0);
    ...
    await page.getByLabel("Screen name", { exact: true }).fill("Muzyka");
    ...
    await page.getByLabel("Screen name", { exact: true }).fill("Rename conflict");
    await page.getByRole("button", { name: "Rename", exact: true }).click();
    ...
    await page.getByLabel("Screen name", { exact: true }).fill("Muzyka");
    await page.getByRole("button", { name: "Rename", exact: true }).click();
    await page.getByLabel("State value 1", { exact: true }).fill("Music");
    ...
    await page.evaluate(async () => { await window.navigation.load(); window.state.subpages = {}; window.state.subpageSavePending = {}; await window.editor.ready(); window.navigation.sync(); });
    await picker.selectOption("1");
    assert.equal(await page.getByLabel("Screen name", { exact: true }).inputValue(), "Muzyka");
    await page.getByLabel("Screen name", { exact: true }).fill("Unsaved name");
    await picker.selectOption("0"); await picker.selectOption("1");
    assert.equal(await page.getByLabel("Screen name", { exact: true }).inputValue(), "Unsaved name");
    ...
    assert.equal(await page.getByLabel("State value 1", { exact: true }).inputValue(), "Music", "restore keeps independent-screen mapping");
    ...
    await page.getByRole("button", { name: "Delete screen", exact: true }).click();
    ...
    assert.equal(await page.getByRole("button", { name: "Delete screen", exact: true }).isVisible(), false, "home cannot be deleted");
    ...
    await add.click();
    await page.getByText("Configuration changed in another browser. Reload before saving again.", { exact: true }).waitFor();
    assert.equal(await picker.inputValue(), "0");
    ...
    assert.equal(await picker.locator("option[value='1']").count(), 0, "quarantine hidden but occupied");
    ...
    await old.locator("#sp-screen-picker").selectOption("2");
    assert.equal(await old.evaluate(() => window.state.editingSubpage), 2, "legacy subpage editing remains available");
