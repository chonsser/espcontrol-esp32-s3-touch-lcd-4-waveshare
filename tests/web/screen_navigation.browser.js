...
(async () => {
  ...
  try {
    ...
    await raw.locator(".sp-screen-active").getByLabel("Screen name", { exact: true }).fill("New name");
    await raw.locator(".sp-screen-active").getByRole("button", { name: "Rename", exact: true }).click();
    ...
    await raw.locator("#sp-screen-picker").selectOption("1");
    await raw.locator(".sp-screen-active").getByLabel("Screen name", { exact: true }).fill("After restore");
    await raw.locator(".sp-screen-active").getByRole("button", { name: "Rename", exact: true }).click();
    ...
    await raw.locator(".sp-screen-active").getByLabel("Screen name", { exact: true }).fill("After generic save");
    await raw.locator(".sp-screen-active").getByRole("button", { name: "Rename", exact: true }).click();
    await raw.waitForFunction(() => window.server.subpages[1] === "@screen:After generic save\n");
    await raw.locator(".sp-screen-active").getByRole("button", { name: "Delete screen", exact: true }).click();
    ...
    await locale.locator(".sp-screen-active").getByLabel("Screen name", { exact: true }).fill("Name draft");
    ...
    assert.equal(await locale.locator(".sp-screen-active").getByLabel("Screen name", { exact: true }).inputValue(), "Name draft");
    const reloaded = locale.waitForEvent("framenavigated");
    await locale.locator(".sp-screen-active").getByRole("button", { name: "Rename", exact: true }).click();
    await reloaded;
    ...
    await busyLocale.locator(".sp-screen-active").getByRole("button", { name: "Rename", exact: true }).click();
    ...
    assert.equal(await page.locator(".sp-screen-active").getByLabel("State value 1", { exact: true }).inputValue(), "Media");
    assert.equal(await page.locator(".sp-screen-active").getByLabel("State value 2", { exact: true }).inputValue(), "Radio");
    await page.locator(".sp-screen-active").getByLabel("State value 2", { exact: true }).selectOption("Radio draft");
    await picker.selectOption("0"); await picker.selectOption("2");
    assert.equal(await page.locator(".sp-screen-active").getByLabel("State value 2", { exact: true }).inputValue(), "Radio draft");
    await add.click();
    ...
    assert.equal(await page.locator(".sp-screen-active").getByLabel("Screen name", { exact: true }).inputValue(), "Muzyka");
    await page.locator(".sp-screen-active").getByLabel("Screen name", { exact: true }).fill("Unsaved name");
    await picker.selectOption("0"); await picker.selectOption("1");
    assert.equal(await page.locator(".sp-screen-active").getByLabel("Screen name", { exact: true }).inputValue(), "Unsaved name");
