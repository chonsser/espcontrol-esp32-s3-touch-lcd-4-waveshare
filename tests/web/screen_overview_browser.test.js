...
test("retired stream metadata cannot add video controls or break screens", async () => {
 ...
 try {
  ...
  await page.evaluate(() => window.seedClockState([
    { id: "select-screensaver_action", state: "HLS Stream", option: ["Display Off", "Screen Dimmed", "Clock", "HLS Stream"] },
    { id: "text-screen_saver_hls_url", state: "http://example.test/retired.m3u8" },
    { id: "text_sensor-screen_saver_hls_status", state: "Stopped" },
  ]));
  for (const id of ["sp-set-clock-mode", "sp-set-sensor-clock-mode"]) {
    assert.deepEqual(await page.locator(`#${id} option`).evaluateAll(options => options.map(option => option.value)), ["off", "dim", "clock"]);
    assert.equal(await page.locator(`#${id}`).inputValue(), "off");
  }
  assert.equal(await page.locator('[id*="hls"]').count(), 0);
  await page.evaluate(() => window.seedClockState([{ id: "select-screen_saver_action", state: "Clock" }]));
  await page.waitForFunction(() => document.getElementById("sp-set-clock-mode").value === "clock");
  await page.getByRole("tab", { name: "Screen", exact: true }).click();
  assert.equal(await page.locator("[data-screen-slot]").count(), 4);
  assert.equal(requests.some(url => /hls|m3u8/i.test(url)), false);
